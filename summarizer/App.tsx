...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen">
      {/* Navigation */}
      <nav className="h-24 flex items-center justify-between px-6 md:px-12 max-w-7xl mx-auto">
        ...
        <div className="flex gap-8 text-[10px] font-bold uppercase tracking-[0.25em] text-slate-500">
      ...
      <main className="max-w-4xl mx-auto px-6 pt-24 pb-48">
        {/* Header Section */}
        <header className="mb-28 text-center">
          <h1 className="text-7xl md:text-9xl font-normal leading-none mb-10 serif italic tracking-tight text-slate-900">
            Distil clarity.
          </h1>
        ...
        <section className="mb-32">
          <form onSubmit={handleSubmit} className="space-y-12">
            <div className="relative group">
              <input
                ...
                className="w-full text-2xl py-8 bg-transparent border-b border-slate-200 focus:border-orange-500 transition-all focus:outline-none placeholder:text-slate-300 placeholder:italic"
                value={state.url}
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-10 pt-4">
              <div className="flex items-center gap-12">
                  ...
                  <div className="flex gap-6">
                    {(Object.keys(ModelType) as Array<keyof typeof ModelType>).map((key) => (
                      <button
                        ...
                        className={`text-xs font-bold uppercase tracking-widest transition-colors ${state.selectedModel === ModelType[key] ? 'text-orange-600' : 'text-slate-400 hover:text-slate-900'}`}
                      >
                        {key}
                      </button>
                    ))}
                  ...
                  <div className="flex gap-6">
                    {(Object.keys(SummaryFormat) as Array<keyof typeof SummaryFormat>).map((key) => (
                      <button
                        ...
                        className={`text-xs font-bold uppercase tracking-widest transition-colors ${state.format === SummaryFormat[key] ? 'text-orange-600' : 'text-slate-400 hover:text-slate-900'}`}
                      >
                        {key.replace('_', ' ')}
                      </button>
                    ))}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <select
                    className="bg-transparent text-xs font-bold cursor-pointer focus:outline-none text-slate-900"
              ...
              <Button type="submit" isLoading={state.isLoading} className="w-full md:w-80 h-16 bg-orange-600 hover:bg-orange-500 rounded-none uppercase text-xs tracking-[0.3em] font-bold shadow-2xl shadow-orange-900/20">
                Run Analysis
              </Button>
            </div>
          </form>
          ...
          {state.error && (
            <div className="mt-12 p-8 border border-red-100 bg-red-50/50 text-red-800 text-sm italic text-center rounded-sm">
              {state.error}
            </div>
          )}
        </section>
        ...
        {/* Synthesis Result */}
        <div ref={summaryRef} className="space-y-40">
          {state.isLoading ? (
            <div className="flex flex-col items-center justify-center py-40 space-y-12">
              <div className="relative w-80 h-px bg-slate-100 overflow-hidden">
              ...
              <div className="grid gap-6">
                {state.history.map((item) => (
                  <button
                    ...
                    className={`bg-white/60 backdrop-blur-md border p-12 text-left transition-all flex justify-between items-center group ${state.currentSummary?.id === item.id ? 'border-orange-500 shadow-xl shadow-orange-900/5' : 'border-slate-100 hover:border-orange-200 hover:bg-white'}`}
                  >
                    <div className="max-w-xl">
                      ...
                      <h4 className="text-3xl serif italic leading-tight text-slate-900 group-hover:text-orange-950 transition-colors">
                        {item.title}
                      </h4>
                    </div>
                    <div className="flex items-center gap-8">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-orange-600 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">View Brief</span>
                      <div className="w-10 h-10 rounded-full border border-slate-100 flex items-center justify-center group-hover:border-orange-200 transition-colors">
      ...
      {/* Deep Footer */}
      <footer className="border-t border-slate-100 py-40 px-6 bg-slate-50/50">
        <div className="max-w-5xl mx-auto">
          <div className="grid md:grid-cols-2 gap-24 mb-32">
            ...
            <div className="grid grid-cols-2 gap-20">
              <div className="space-y-8">
                ...
                <ul className="text-xs text-slate-500 space-y-5">
          ...
          <div className="pt-16 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-8 text-[10px] font-bold text-slate-400 uppercase tracking-[0.4em]">
