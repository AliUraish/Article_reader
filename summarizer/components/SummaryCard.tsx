...
export const SummaryCard: React.FC<SummaryCardProps> = ({ record, onDelete }) => {
  ...
  return (
    <div className="bg-white border border-slate-100 p-6 sm:p-10 md:p-20 shadow-2xl shadow-slate-200/50 rounded-sm">
      <div className="max-w-3xl mx-auto">
        <header className="mb-8 sm:mb-14 pb-8 sm:pb-14 border-b border-slate-100">
          ...
          <h2 className="text-3xl sm:text-5xl md:text-7xl font-normal text-slate-900 leading-[1.1] mb-6 sm:mb-8 serif italic">
            {record.title}
          </h2>
          
          <div className="flex flex-wrap items-center gap-4 sm:gap-6 text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em]">
        ...
        <article className="prose prose-slate max-w-none">
          <div className="text-slate-800 leading-relaxed sm:leading-[2] text-base sm:text-xl font-[400] whitespace-pre-wrap selection:bg-orange-100 selection:text-orange-950">
            {record.summary}
          </div>
        </article>

        <footer className="mt-12 sm:mt-20 pt-8 sm:pt-10 border-t border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6 sm:gap-8">
          <div className="flex items-center gap-4 text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-[0.35em] sm:tracking-[0.4em]">
            <div className="w-1.5 h-1.5 rounded-full bg-orange-500"></div>
            <span>Generated via Gemini Synthesis</span>
          </div>
          <div className="px-4 py-1.5 border border-slate-100 rounded-sm text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-widest">
